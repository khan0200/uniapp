...
window.toggleSortOrder = function(tab) {
  if (tab === 'students') {
    ...
    documentsSortOrder = documentsSortOrder === 'asc' ? 'desc' : 'asc';
    const icon = document.getElementById("documentsSortIcon");
    if (icon) icon.className = documentsSortOrder === 'asc' ? "bi bi-chevron-down ms-1" : "bi bi-chevron-up ms-1";
    if (typeof filterDocuments === 'function') filterDocuments(true);
  }
};
...
const ADMISSION_COLORS = {
  "1 MONTH KDB": {
    ...
    bd: "rgba(139, 92, 246, 0.3)",
  },
  "1 MONTH KDB OK": {
    text: "#4338ca",
    ball: "#6366f1",
    bg: "rgba(99, 102, 241, 0.15)",
    bd: "rgba(99, 102, 241, 0.3)",
  },
  "COA RECEIVED": {
    text: "#15803d",
    ball: "#22c55e",
...
function getUniversitiesForLevel(level) {
  if (!window.universitiesData) return [];
  const universities = window.universitiesData.map((u) => u.name);
  // Return unique, sorted list
  return Array.from(new Set(universities)).sort((a, b) => a.localeCompare(b));
}
...
function showTab(tabName) {
  const tabs = [
    "students-tab", "documents-tab", "status-tab", "payments-tab", 
    "admissions-tab", "notifications-tab", "settings-tab", "tasks-tab"
  ];
  ...
  // Render documents when documents tab is activated
...
function getDocRemainingCount(s, docName) {
  ...
  let key;
  if (docName === "BIRTH CERTIFICATE") key = "bcHandCount";
  else if (docName === "MARRIAGE CERTIFICATE") key = "mcHandCount";
  else if (docName === "APOSTILLE") key = "aposHandCount";
  else if (docName === "3.5x4.5") key = "picHandCount";
  ...
  let used = 0;
  if (s.university1 && s.university1.trim() !== "") used++;
  if (s.university2 && s.university2.trim() !== "") used++;
...
function getDocStatusHtml(s, docName) {
  ...
  if (isMissing) {
    return `<span class="badge doc-missing-badge rounded-circle d-inline-flex align-items-center justify-content-center shadow-sm" style="width: 24px; height: 24px; font-weight: 700; font-size: 0.8rem;" title="Missing / 0 copies left">0</span>`;
  }
...
function filterDocuments(resetPage = true) {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML =
      levelFilter === "DELETED"
        ? '<tr><td colspan="6" class="text-center py-5 text-secondary"><i class="bi bi-trash me-2"></i>No deleted students.</td></tr>'
        : '<tr><td colspan="6" class="text-center py-5 text-secondary"><i class="bi bi-folder-x me-2"></i>No students match your filters.</td></tr>';
    const pag = document.getElementById("documentsPagination");
    if (pag) pag.innerHTML = "";
    return;
  }

  // Clear pagination since all students are displayed on a single scrollable page
  const pag = document.getElementById("documentsPagination");
  if (pag) pag.innerHTML = "";
  ...
  tbody.innerHTML = filtered.map((student) => {
    ...
    if (missingDocs.includes("FULL OK")) {
         ...
         const itemsToRender = missingDocs.filter(doc => {
           ...
           if (doc === "MARRIAGE CERTIFICATE" && s.hasMc === false) return false;
           return true;
         });
         
         if (itemsToRender.length > 0) {
           let listHtml = '<div class="d-flex flex-wrap gap-1 align-items-center mt-1">';
           itemsToRender.forEach(doc => {
             ...
             listHtml += `<span style="${getDocPillStyle(label)}">${label}</span>`;
           });
           listHtml += '</div>';
           docsHtml = listHtml;
         }
    ...
    const isBcMissing = !missingDocs.includes("FULL OK") && missingDocs.includes("BIRTH CERTIFICATE");
    const isMcMissing = s.hasMc !== false && !missingDocs.includes("FULL OK") && missingDocs.includes("MARRIAGE CERTIFICATE");
    const isAposMissing = !missingDocs.includes("FULL OK") && missingDocs.includes("APOSTILLE");
    const isPicMissing = !missingDocs.includes("FULL OK") && missingDocs.includes("3.5x4.5");

    const bcClass = isBcMissing ? 'text-center doc-missing-cell' : 'text-center';
    const mcClass = isMcMissing ? 'text-center doc-missing-cell' : 'text-center';
    const aposClass = isAposMissing ? 'text-center doc-missing-cell' : 'text-center';
    const picClass = isPicMissing ? 'text-center doc-missing-cell' : 'text-center';
    ...
    return `
      <tr class="student-table-row ${s.deleted ? "deleted-row" : ""}" style="cursor: pointer;"
          onclick="viewDocumentsModal('${uniqueId}')">
        <!-- Column 1: Full Name with Student ID, Tariff, and Education Level as sub-text -->
        <td class="student-name-cell">
            ${s.deleted ? '<i class="bi bi-trash text-danger me-1" style="font-size:0.75rem;"></i>' : ""}
            <span class="table-full-name">${s.fullName || "-"}</span>
            ${ghostText ? `<span class="table-tariff-ghost" style="margin-top: 4px; display: block; font-size: 0.8rem; opacity: 0.85;">${ghostText}</span>` : ""}
        </td>
        
        <!-- Column 2: Missing (Other Documents) -->
        <td>
           ${docsHtml}
        </td>
        ...
        <!-- Column 3: BC -->
        <td class="${bcClass}">
           ${getDocStatusHtml(s, "BIRTH CERTIFICATE")}
        </td>

        <!-- Column 4: MC -->
        <td class="${mcClass}">
           ${getDocStatusHtml(s, "MARRIAGE CERTIFICATE")}
        </td>

        <!-- Column 5: APOS -->
        <td class="${aposClass}">
           ${getDocStatusHtml(s, "APOSTILLE")}
        </td>

        <!-- Column 6: PIC -->
        <td class="${picClass}">
           ${getDocStatusHtml(s, "3.5x4.5")}
        </td>
      </tr>
    `;
  }).join("");
...
function viewDocumentsModal(uniqueId) {
  ...
  const titleEl = document.getElementById("documentsModalLabel");
  if (titleEl) {
  ...
  if (missingContainer) {
    ...
    if (missingList.length === 0) {
      ...
      let missingHtml = '<div class="d-flex flex-column gap-2">';
  ...
  if (pickNeededContainer) {
    ...
    const selectedPills = s.pickNeeded || [];
    
    let pillsHtml = '<div class="d-flex flex-wrap gap-2">';
    ...
    pickNeededContainer.innerHTML = pillsHtml;
  }

  // Populate Manual Copy Override Section
  const copyContainer = document.getElementById("copyCounterContainer");
  if (copyContainer) {
    const docsToManage = [
      { key: "bcHandCount", docName: "BIRTH CERTIFICATE", label: "BC" },
      { key: "mcHandCount", docName: "MARRIAGE CERTIFICATE", label: "MC" },
      { key: "aposHandCount", docName: "APOSTILLE", label: "APOS" },
      { key: "picHandCount", docName: "3.5x4.5", label: "PIC" }
    ];
    ...
    docsToManage.forEach(item => {
      ...
      if (isMcDisabled) {
        currentVal = "N/A";
      } else if (s[item.key] !== undefined && s[item.key] !== null) {
        currentVal = parseInt(s[item.key], 10);
      } else {
        if (isMissing) {
          currentVal = 0;
        } else {
          ...
          if (s.university1 && s.university1.trim() !== "") used++;
          if (s.university2 && s.university2.trim() !== "") used++;
          currentVal = Math.max(0, 3 - used);
        }
      }
      
      // Disabled if document is missing (value remains 0) or if MC is disabled
      const isDisabled = (isMissing || isMcDisabled) ? "disabled" : "";
        ...
        toggleHtml = `
          <div style="position: absolute; top: 6px; right: 6px; z-index: 5;">
            <div class="form-check form-switch p-0 m-0">
              <input class="form-check-input" type="checkbox" role="switch" id="mcToggle-${uniqueId}" 
                     ${s.hasMc !== false ? "checked" : ""} 
                     onchange="toggleMcEnabled('${uniqueId}')"
                     title="${s.hasMc !== false ? 'Marriage Certificate required' : 'Marriage Certificate not applicable'}"
                     style="width: 26px; height: 14px; margin: 0; cursor: pointer;">
            </div>
          </div>
        `;
      }
      ...
      containerHtml += `
        <div class="col">
          <div class="d-flex flex-column align-items-center p-3 rounded border" style="position: relative; background: var(--bg-primary); border-color: var(--border-color); ${isMcDisabled ? 'opacity: 0.75;' : ''}">
            ${toggleHtml}
            <span class="fw-bold text-secondary mb-2" style="font-size: 0.75rem; letter-spacing: 0.05em; padding-right: ${item.label === 'MC' ? '18px' : '0'};">${item.label}</span>
            <div class="d-flex align-items-center gap-2">
              <button class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center p-0 rounded-circle" 
                      style="width: 24px; height: 24px; font-weight: bold; border-width: 1.5px;"
                      onclick="updateCopyCount('${uniqueId}', '${item.key}', ${isMcDisabled ? 0 : currentVal - 1})"
                      ${isDisabled || currentVal <= 0 ? "disabled" : ""}>-</button>
              <span class="fw-bold mx-1" style="font-size: 1.15rem; color: ${isMissing || isMcDisabled ? 'var(--text-secondary)' : 'var(--accent-primary)'}; width: 30px; text-align: center;">${currentVal}</span>
              <button class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center p-0 rounded-circle" 
                      style="width: 24px; height: 24px; font-weight: bold; border-width: 1.5px;"
                      onclick="updateCopyCount('${uniqueId}', '${item.key}', ${isMcDisabled ? 0 : currentVal + 1})"
    ...
    copyContainer.innerHTML = containerHtml;
  }
...
// Toggle pick needed pill status
// Helper to show a reminder modal with custom message
...
window.togglePickNeeded = function(uniqueId, pill) {
  ...
  const s = window.studentsData[index];
  if (!s.pickNeeded) s.pickNeeded = [];
  
  if (pill === "FULL OK") {
    if (s.pickNeeded.includes("FULL OK")) {
      s.pickNeeded = [];
    } else {
      s.pickNeeded = ["FULL OK"];
    }
  } else {
    s.pickNeeded = s.pickNeeded.filter(p => p !== "FULL OK");
    if (s.pickNeeded.includes(pill)) {
      s.pickNeeded = s.pickNeeded.filter(p => p !== pill);
      
      // SHOW REMINDER MESSAGE
      if (pill === "Foreign passport") {
        showReminder("Talabani zagranini oldingizmi? uni kirgizing, ismini tekshirib yozib qo'ying!");
      } else if (pill === "2 ta nomer") {
        showReminder("Talabaning nomerini to'g'ri yozing! Bazaga kiritish esdan chiqmasin!");
      } else if (pill === "Email") {
        showReminder("Email ni tekshirib oling, Bazaga kiritish esdan chiqmasin! Iltimos");
      } else if (pill === "Manzil") {
        showReminder("Manzil ingliz tilida yozing iltimos. Bazaga kiritish esdan chiqmasin");
      } else if (["IELTS", "TOEFL", "SKA", "TOPIK", "SAT", "CEFR"].includes(pill)) {
        showReminder("Til sertifikati va darajasini talabani bazasiga yozib qo'ying!");
      }
    } else {
      s.pickNeeded.push(pill);
    }
  }
  ...
  if (typeof updateStudentInFirestore === "function" && s.firestoreId) {
    updateStudentInFirestore(s.firestoreId, { pickNeeded: s.pickNeeded });
  }
  
  // Re-render modal to reflect changes
  viewDocumentsModal(uniqueId);
  
  // Update documents table to reflect changes without resetting page
  if (typeof filterDocuments === 'function') {
    filterDocuments(false);
  }
};

// Toggle Marriage Certificate applicability status
window.toggleMcEnabled = function(uniqueId) {
  let index = window.studentsData.findIndex((student) => student.firestoreId === uniqueId);
  if (index === -1) {
    index = window.studentsData.findIndex((student) => student.id === uniqueId);
  }
  ...
  const s = window.studentsData[index];
  s.hasMc = s.hasMc === false ? true : false;
  ...
  // Save to Firestore if available
  if (typeof updateStudentInFirestore === "function" && s.firestoreId) {
    updateStudentInFirestore(s.firestoreId, { hasMc: s.hasMc });
  }
...
// Increment / Decrement physical copies manually
window.updateCopyCount = function(uniqueId, key, newVal) {
  let index = window.studentsData.findIndex((student) => student.firestoreId === uniqueId);
  if (index === -1) {
    index = window.studentsData.findIndex((student) => student.id === uniqueId);
  }
  ...
  if (typeof updateStudentInFirestore === "function" && s.firestoreId) {
    ...
    updateStudentInFirestore(s.firestoreId, updateData);
  }
...
// Reset copies manual override back to auto-calculation
window.resetCopyCount = function(uniqueId, key) {
  let index = window.studentsData.findIndex((student) => student.firestoreId === uniqueId);
  if (index === -1) {
    index = window.studentsData.findIndex((student) => student.id === uniqueId);
  }
...
// Helper to automatically track missing documents based on field values
function updateAutoDocuments(s) {
  if (!s) return false;
  let changed = false;
  const missingDocs = s.pickNeeded || [];
  
  const check = (condition, docName) => {
    const index = missingDocs.indexOf(docName);
    if (condition) {
      if (index === -1) {
  ...
  const phone1Empty = !s.phone1 || s.phone1 === "-" || s.phone1.trim() === "";
  const phone2Empty = !s.phone2 || s.phone2 === "-" || s.phone2.trim() === "";
  check(phone1Empty && phone2Empty, "2 ta nomer");

  // 2. Email check
  check(!s.email || s.email === "-" || s.email.trim() === "", "Email");

  // 3. Passport check -> "Foreign passport"
  check(!s.passport || s.passport === "-" || s.passport.trim() === "", "Foreign passport");
  ...
  if (changed) {
    ...
    // Refresh table if documents tab is active
    if (document.getElementById("documents-tab").style.display !== "none") {
        renderDocuments();
    }
  }
  return changed;
...
function viewStudentDetails(uniqueId) {
  ...
  // Auto-update document status based on fields
  updateAutoDocuments(s);
  ...
  const tariffOptions =
    ...
    (window.tariffsData || [])
      .map(
        (t) =>
          `<option value="${t.name}" ${s.tariff === t.name ? "selected" : ""}>${t.name}</option>`,
      )
      .join("");
  ...
  const levelOptions =
    ...
    (window.levelsData || [])
      .map(
        (l) =>
          `<option value="${l.name}" ${s.level === l.name ? "selected" : ""}>${l.name}</option>`,
      )
      .join("");
  ...
  const groupOptions = (window.groupsData || [])
    .map(
      (g) =>
        `<option value="${g.name}" ${s.group === g.name ? "selected" : ""}>${g.name}</option>`,
    )
    .join("");
  ...
  const detailsHtml = `
    <div class="sd3-profile-head">
      <div class="sd3-avatar">${studentInitials || "?"}</div>
      <div class="sd3-profile-main">
        <div class="sd3-profile-title">${s.fullName || "Unnamed Student"}</div>
        <div class="sd3-profile-meta">
          <span>Student ID: ${s.id || "-"}</span>
          <span class="sd3-status-chip">${s.deleted ? "Deleted" : "Active"}</span>
    ...
    <div class="sd3-wrap">
      ...
      <div class="col-md-5 sd3-col">
        <div class="row g-2">
          <div class="col-12"><div class="detail-group editable ${(!s.fullName || s.fullName === "") ? "missing-field-highlight" : ""}" data-field="fullName"><label class="detail-label"><i class="bi bi-person-fill me-1" style="color:#5b8def;"></i>Full Name</label><div class="detail-value-wrap"><span class="detail-value">${s.fullName}</span><div class="action-btns"><button class="copy-btn" onclick="copyToClipboard('${s.fullName.replace(/'/g,"\\'")}',this);event.stopPropagation();"><i class="bi bi-clipboard"></i></button><button class="edit-btn" onclick="startEdit('fullName','text');event.stopPropagation();"><i class="bi bi-pencil"></i></button></div></div><div class="edit-field" style="display:none;"><input type="text" class="form-control ios-input form-control-sm" value="${s.fullName}" id="edit-fullName" style="text-transform:uppercase;"><div class="edit-actions"><button class="save-btn" onclick="saveEdit('fullName')"><i class="bi bi-check"></i></button><button class="cancel-btn" onclick="cancelEdit('fullName')"><i class="bi bi-x"></i></button></div></div></div></div>
          ${(() => {
            ...
            return `
              <div class="col-6 mt-1">
                <div class="detail-group">
                  <label class="detail-label"><i class="bi bi-person-fill me-1" style="color:#5b8def;"></i>Family Name <span class="badge badge-auto ms-1" style="vertical-align: middle;">Auto</span></label>
              ...
              <div class="col-6 mt-1">
                <div class="detail-group">
                  <label class="detail-label"><i class="bi bi-person-fill me-1" style="color:#5b8def;"></i>Given Name <span class="badge badge-auto ms-1" style="vertical-align: middle;">Auto</span></label>
                  <div class="detail-value-wrap">
            ...
            [1, 2, 3].map(function(n) {
              ...
              return '<div class="col-4">' +
                '<div class="detail-group editable ' + (isMissing ? 'missing-field-highlight' : '') + '" data-field="university' + n + '">' +
                  '<label class="detail-label"><i class="bi bi-building-fill me-1" style="color:#5b8def;"></i>University ' + n + '</label>' +
                  '<div class="detail-value-wrap">' +
                    '<div style="display:flex;flex-direction:column;gap:0;flex:1;min-width:0;">' +
                      '<span class="detail-value">' + (uniVal || '-') + '</span>' +
                      '<span class="uni-status-badge uni-status--' + uniSt + '"' +
                        ' onclick="openUniStatusPopup(this, \'university' + n + 'Status\', \'' + uniSt + '\'); event.stopPropagation();">' +
                        '<i class="bi bi-circle-fill" style="font-size:0.45rem;"></i>' + uniSt +
                      '</span>' +
                    '</div>' +
                    '<button class="edit-btn" onclick="startEdit(\'university' + n + '\', \'select\'); event.stopPropagation();"><i class="bi bi-pencil"></i></button>' +
                  '</div>' +
                  '<div class="edit-field" style="display:none;">' +
                    '<select class="form-select ios-input form-control-sm" id="edit-university' + n + '">' +
                      '<option value="">Choose University</option>' +
                      uniOptions +
                    '</select>' +
                    '<div class="edit-actions">' +
                      '<button class="save-btn" onclick="saveEdit(\'university' + n + '\')"><i class="bi bi-check"></i></button>' +
                      '<button class="cancel-btn" onclick="cancelEdit(\'university' + n + '\')"><i class="bi bi-x"></i></button>' +
                    '</div>' +
                  '</div>' +
                '</div>' +
              '</div>';
          ...
          <div class="col-12"><div class="detail-group editable ${(!s.address || s.address === "-" || s.address === "") ? "missing-field-highlight" : ""}" data-field="address"><label class="detail-label"><i class="bi bi-geo-alt-fill me-1" style="color:#5b8def;"></i>Address</label><div class="detail-value-wrap"><span class="detail-value">${s.address||"-"}</span><div class="action-btns">${s.address?`<button class="copy-btn" onclick="copyToClipboard(\`${(s.address||"").replace(/`/g,"\\`").replace(/'/g,"\\'")}\`,this);event.stopPropagation();"><i class="bi bi-clipboard"></i></button>`:""}<button class="edit-btn" onclick="startEdit('address','textarea');event.stopPropagation();"><i class="bi bi-pencil"></i></button></div></div><div class="edit-field" style="display:none;"><textarea class="form-control ios-input form-control-sm" id="edit-address" rows="2" style="text-transform:uppercase;">${s.address||""}</textarea><div class="edit-actions"><button class="save-btn" onclick="saveEdit('address')"><i class="bi bi-check"></i></button><button class="cancel-btn" onclick="cancelEdit('address')"><i class="bi bi-x"></i></button></div></div></div></div>
          <div class="col-12"><div class="detail-group editable" data-field="notes"><label class="detail-label"><i class="bi bi-sticky-fill me-1" style="color:#5b8def;"></i>Notes ${s.noteImportance?`<span class="badge rounded-pill ms-2" style="background:${s.noteImportance==="GREEN"?"#28a745":s.noteImportance==="YELLOW"?"#ffc107":"#dc3545"};color:white;font-size:0.7rem;">${s.noteImportance}</span>`:""}</label><div class="detail-value-wrap"><span class="detail-value">${s.notes||"-"}</span><button class="edit-btn" onclick="startEdit('notes','textarea');event.stopPropagation();"><i class="bi bi-pencil"></i></button></div><div class="edit-field" style="display:none;"><textarea class="form-control ios-input form-control-sm mb-2" id="edit-notes" rows="2">${s.notes||""}</textarea><select class="form-select ios-input form-control-sm mb-2" id="edit-noteImportance"><option value="">No Priority</option><option value="GREEN" ${s.noteImportance==="GREEN"?"selected":""}>🟢 GREEN (Low)</option><option value="YELLOW" ${s.noteImportance==="YELLOW"?"selected":""}>🟡 YELLOW (Medium)</option><option value="RED" ${s.noteImportance==="RED"?"selected":""}>🔴 RED (High)</option></select><div class="edit-actions"><button class="save-btn" onclick="saveEdit('notes')"><i class="bi bi-check"></i></button><button class="cancel-btn" onclick="cancelEdit('notes')"><i class="bi bi-x"></i></button></div></div></div></div>
        </div>
      </div>
      <div class="col-md-3 sd3-col">
        ${(()=>{
          const allPayments=window.paymentsData||[];
          const studentPayments=allPayments.filter(p=>p.studentFirestoreId===s.firestoreId&&!p.isWithdrawal&&!p.isDiscount&&parseFloat(p.amount)>0);
          const paymentsDone=studentPayments.reduce((sum,p)=>sum+(parseFloat(p.amount)||0),0);
          ...
          return `<div class="d-flex flex-column gap-2">
            <div class="detail-group editable ${(!s.office || s.office === "" || s.office === "—" || s.office === "-") ? "missing-field-highlight" : ""}" data-field="office" style="border-left:3px solid #5b8def;">
              <label class="detail-label"><i class="bi bi-geo-alt-fill me-1" style="color:#ffffff;"></i>Office</label>
            ...
            <div class="detail-group pmt-box ${balClass}">
              <label class="detail-label"><i class="bi bi-wallet-fill me-1"></i>Student Balance</label>
            ...
            <div class="detail-group pmt-box pmt-done-box">
              <label class="detail-label"><i class="bi bi-check-circle-fill me-1"></i>Payments Done</label>
            ...
            <div class="detail-group pmt-box discount-box">
              <label class="detail-label"><i class="bi bi-tag-fill me-1"></i>Discount</label>
  ...
  if (s.university1)
    document.getElementById("edit-university1").value = s.university1;
  if (s.university2)
    document.getElementById("edit-university2").value = s.university2;
...
function showAuthModal(callback, title = "Enter Password") {
  ...
  if (!modalEl) {
    ...
    div.innerHTML = `
      <div class="modal fade" id="authModal" tabindex="-1" aria-labelledby="authModalLabel" aria-hidden="true" style="background: rgba(15, 23, 42, 0.75) !important; backdrop-filter: blur(8px) !important; z-index: 1070 !important;">
        <div class="modal-dialog modal-dialog-centered modal-sm" style="z-index: 1071;">
          <div class="modal-content" style="background: #ffffff; border: 2px solid var(--bs-primary, #0d6efd); border-radius: 16px; box-shadow: 0 25px 60px rgba(0,0,0,0.4);">
            <div class="modal-header border-0 pb-0">
              <h6 class="modal-title fw-bold" id="authModalLabel" style="font-size: 1.15rem; color: #1e293b;"><i class="bi bi-shield-lock-fill me-2 text-primary" style="font-size: 1.3rem;"></i><span id="authModalTitle">Enter Password</span></h6>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body py-4 px-4">
              <div class="form-group mb-4">
                <input type="password" class="form-control form-control-lg" id="authModalPassword" placeholder="••••" style="letter-spacing: 0.3rem; font-size: 1.4rem; text-align: center; border-radius: 10px; border: 2px solid #e2e8f0; background: #f8fafc; color: #0f172a; font-weight: bold;">
              </div>
              <button type="button" class="btn btn-primary btn-lg w-100 fw-bold py-2 shadow-sm" id="authModalSubmit" style="border-radius: 10px;">Confirm</button>
            </div>
          </div>
        </div>
      </div>
  ...
  modalEl.addEventListener("shown.bs.modal", () => pwdInput.focus(), { once: true });

  const submitBtn = document.getElementById("authModalSubmit");
  const onSubmit = () => {
    if (pwdInput.value === "1198") {
      modal.hide();
...
function startEdit(field, type) {
  const isAdmin = window.location.pathname.includes("management.html") || window.location.href.includes("management.html");
  if (!isAdmin && window.editAuthFlag === 0) {
    showAuthModal(() => startEdit(field, type), "Enter Password to Edit");
    return;
  }
...
function saveEdit(field) {
  ...
  if (["fullName", "passport", "address", "sex"].includes(field)) {
  ...
  let s;
  if (window.currentStudentFirestoreId) {
    s = window.studentsData.find(
      (student) => student.firestoreId === window.currentStudentFirestoreId,
    );
  }
  ...
  // Refresh modal UI to show highlights and updated state
  if (s.firestoreId) {
    viewStudentDetails(s.firestoreId);
  }
  ...
  if (valueSpan) {
    if (field === "languageCertificate" || field === "languageCertificate2" || field === "languageCertificate3") {
      ...
      const certLower = (newValue || "").toLowerCase().replace(/\s+/g, "-");
      valueSpan.innerHTML = `<span class="badge badge-language badge-cert-${certLower}">${newValue}</span>${scoreValue ? ` <span class="badge badge-score">Score: ${scoreValue}</span>` : ""}`;
    } else if (field === "level" || field === "level2" || field === "group") {
      if (!newValue || newValue === "-" || (field === "group" && newValue === "No Group")) {
        valueSpan.innerHTML = field === "group" ? `<span class="badge badge-group">No Group</span>` : "-";
      } else {
        const badgeClass = (field === "level" || field === "level2") ? "badge-level" : "badge-group";
        const levelLower = (field === "level" || field === "level2") ? `badge-level-${newValue.toLowerCase().replace(/\s+/g, "-")}` : "";
        valueSpan.innerHTML = `<span class="badge ${badgeClass} ${levelLower}">${newValue}</span>`;
      }
...
const UNIVERSITY_STATUS_OPTIONS = [
  { value: "Chosen", color: "#2563eb" },
  { value: "Applied", color: "#d97706" },
  { value: "Accepted", color: "#059669" },
  { value: "Failed", color: "#dc2626" },
];

function getCurrentStudentForModal() {
  let student;
  if (window.currentStudentFirestoreId) {
    student = window.studentsData.find(
      (item) => item.firestoreId === window.currentStudentFirestoreId,
    );
  }
  return student || window.studentsData[currentStudentId];
}

function openUniStatusPopup(trigger, field, currentValue) {
  const isAdmin = window.location.pathname.includes("management.html") || window.location.href.includes("management.html");
  if (!isAdmin && window.editAuthFlag === 0) {
    showAuthModal(() => openUniStatusPopup(trigger, field, currentValue), "Enter Password to Edit");
    return;
  }

  const popup = document.getElementById("uniStatusPopup");
  if (!popup) {
    showNotification("University status popup not found!", "error");
    return;
  }

  const activeValue = currentValue || "Chosen";
  popup.innerHTML = `
    <div class="uni-status-popup-header">University Status</div>
    ${UNIVERSITY_STATUS_OPTIONS.map(
      (option) => `
        <div class="uni-status-popup-option ${option.value === activeValue ? "usp-active" : ""}"
          onclick="setUniStatus('${field}', '${option.value}')">
          <span class="uni-status-popup-dot" style="background:${option.color};"></span>
          <span class="uni-status-popup-label">${option.value}</span>
        </div>
      `,
    ).join("")}
  `;
...
function setUniStatus(field, value) {
  const s = getCurrentStudentForModal();
  if (!s) {
    showNotification("Error: Student not found!", "error");
    return;
  }
...
document.addEventListener("click", function (event) {
  ...
  if (
    ...
    !event.target.closest(".uni-status-badge")
  ) {
    popup.style.display = "none";
  }
});

window.openUniStatusPopup = openUniStatusPopup;
window.setUniStatus = setUniStatus;
...
function permanentlyDeleteStudent() {
  ...
  let studentIndex = -1;
  if (window.currentStudentFirestoreId) {
    studentIndex = window.studentsData.findIndex(
      (student) => student.firestoreId === window.currentStudentFirestoreId,
    );
    if (studentIndex !== -1) {
  ...
  if (
    !confirm(
      `Are you sure you want to PERMANENTLY delete "${s.fullName}"?\n\nâš ï¸ This action cannot be undone!`,
    )
  ) {
    return;
  }
    ...
    tbody.innerHTML = paginatedData
      .map((s) => {
        ...
        if (s.languageCertificate && s.languageCertificate !== "NO CERTIFICATE") {
          const certLower = s.languageCertificate.toLowerCase().replace(/\s+/g, "-");
          certHtml = `<div class="mt-1 d-flex gap-1 align-items-center flex-wrap"><span class="badge badge-language badge-cert-${certLower}" style="font-size: 0.58rem; padding: 0.12rem 0.4rem; text-transform: uppercase; border-radius: 4px;">${s.languageCertificate}</span>`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Debounce utility for documents search input
  let documentsSearchDebounceTimer = null;
  function debouncedFilterDocuments() {
    clearTimeout(documentsSearchDebounceTimer);
    documentsSearchDebounceTimer = setTimeout(() => filterDocuments(), 300);
  }

  // Documents tab search and filter listeners
  const documentsSearchInput = document.getElementById("documentsSearchInput");
  if (documentsSearchInput) {
    documentsSearchInput.addEventListener("input", debouncedFilterDocuments);
  }

  const documentsFilterTariff = document.getElementById("documentsFilterTariff");
  if (documentsFilterTariff) {
    documentsFilterTariff.addEventListener("change", () => filterDocuments());
  }

  const documentsFilterLevel = document.getElementById("documentsFilterLevel");
  if (documentsFilterLevel) {
    documentsFilterLevel.addEventListener("change", () => filterDocuments());
  }

  const documentsFilterGroup = document.getElementById("documentsFilterGroup");
  if (documentsFilterGroup) {
    documentsFilterGroup.addEventListener("change", () => filterDocuments());
  }

  const documentsFilterLanguageCertificate = document.getElementById("documentsFilterLanguageCertificate");
  if (documentsFilterLanguageCertificate) {
    documentsFilterLanguageCertificate.addEventListener("change", () => filterDocuments());
  }
  ...
  if (statusInvoice)
    statusInvoice.addEventListener("change", applyStatusFilters);
  const statusAdmission = document.getElementById("statusFilterAdmission");
  if (statusAdmission)
    statusAdmission.addEventListener("change", applyStatusFilters);
...
function openStatusDropdown(triggerEl, type, uniqueId) {
  ...
  const fieldName =
    type === "invoice" ? "invoiceStatuses" : "admissionStatuses";
...
function toggleStatusOption(checkbox) {
  ...
  const trigger = document.querySelector(
    `.sms-trigger[data-uid="${uniqueId}"][data-type="${type}"]`,
  );
  if (trigger) {
    trigger.innerHTML =
      renderStatusBadges(type, current) +
...
function applyStatusFilters(resetPage = true) {
  ...
  const filtered = window.studentsData.filter((s) => {
    ...
    if (!searchQuery) {
      ...
      if (searchType === "all") {
        matchesSearch = nameMatch || idMatch || emailMatch || uniMatch || langMatch;
      } else if (searchType === "id") {
        matchesSearch = idMatch;
      } else if (searchType === "name") {
        matchesSearch = nameMatch;
  ...
  tbody.innerHTML = paginatedData
    .map((s) => {
      ...
      const invStatuses = Array.isArray(s.invoiceStatuses)
        ? s.invoiceStatuses
        : [];
      const admStatuses = Array.isArray(s.admissionStatuses)
        ? s.admissionStatuses
        : [];
...
function downloadSelectedAsExcel() {
  ...
  const modal = bootstrap.Modal.getInstance(
    document.getElementById("downloadExcelModal"),
  );
  modal.hide();

  showNotification(
    `Downloaded ${selectedStudents.length} students to Excel!`,
    "success",
  );
}
...
function downloadPaymentHistoryAsExcel() {
  ...
  showNotification(
    `Downloaded ${payments.length} payment records to Excel!`,
    "success",
  );
}
...
function renderPaymentStudents(filteredData = null) {
  ...
  const students =
    filteredData || window.studentsData.filter((s) => !s.deleted);

  // Update Counter
  const counterEl = document.getElementById("paymentStudentsCounter");
  if (counterEl) {
    // Check if filters are active by checking input values
    const searchTerm = document.getElementById("paymentSearchInput")
      ? document.getElementById("paymentSearchInput").value
      : "";
    const tariffFilter = document.getElementById("paymentTariffFilter")
      ? document.getElementById("paymentTariffFilter").value
      : "";
  ...
  if (!students || students.length === 0) {
    container.innerHTML = `
        ...
        `;
    renderPaginationControls(
      "paymentStudentsPagination",
      1,
      0,
      ITEMS_PER_PAGE,
      "changePaymentStudentsPage",
    );
    return;
  }
  ...
  container.innerHTML = paginatedData
    .map((s) => {
      ...
      const discount = parseFloat(s.discount) || 0;
      const balanceClass =
        balance > 0 ? "bal-positive" : balance < 0 ? "bal-negative" : "";
...
function renderPaymentHistory(filteredData = null) {
  ...
  if (counterEl) {
    ...
    const searchTerm = document.getElementById("paymentHistorySearch")
      ? document.getElementById("paymentHistorySearch").value
      : "";
    const methodFilter = document.getElementById("paymentMethodFilter")
      ? document.getElementById("paymentMethodFilter").value
      : "";
    const receiverFilter = document.getElementById("receivedByFilter")
      ? document.getElementById("receivedByFilter").value
      : "";
    ...
    .map((p) => {
      ...
      if (p.notes) {
        ...
        if (p.notes.toUpperCase().includes("DISCOUNT")) {
          noteStyle =
            "background-color: rgba(219, 39, 119, 0.15); color: #be185d; border: 1px solid rgba(219, 39, 119, 0.25);";
        } else if (p.notes.toLowerCase().includes("shartnoma")) {
          noteStyle =
            "background-color: rgba(6, 182, 212, 0.15); color: #0891b2; border: 1px solid rgba(6, 182, 212, 0.25);";
        } else {
          noteStyle =
            "background-color: rgba(107, 114, 128, 0.15); color: #4b5563; border: 1px solid rgba(107, 114, 128, 0.25);";
        }
      ...
      return `
        <div class="col-12 col-md-6 col-lg-4">
            <div class="payment-history-card" onclick="viewPaymentDetails('${p.firestoreId}')" style="cursor: pointer;">
                ...
                <div class="payment-actions mt-2 d-flex gap-3 justify-content-end">
                    ...
                    <button class="btn btn-link text-decoration-none p-0 fw-semibold" style="font-size: 0.8rem; color: #10b981;" onclick="event.stopPropagation(); printPaymentCheque('${p.firestoreId}')">
...
function populateStudentDropdown() {
  ...
  dropdown.innerHTML =
    ...
    students
      .map(
        (s) =>
          `<option value="${s.firestoreId || s.id}" data-student-id="${s.id}" data-student-name="${s.fullName}">${s.id} - ${s.fullName}</option>`,
      )
      .join("");
}
...
function updatePaymentStudentInfo() {
  ...
  if (leftPanel) {
    ...
    if (psiLangCertEl) {
      if (s.languageCertificate && s.languageCertificate !== "NO CERTIFICATE") {
        const certLower = s.languageCertificate.toLowerCase().replace(/\s+/g, "-");
        let html = `<span class="badge badge-language badge-cert-${certLower}" style="font-size:0.68rem; padding:0.15rem 0.45rem;">${s.languageCertificate}</span>`;
        if (s.certificateScore) {
...
function submitWithdrawal() {
  ...
  // Close modal and reset form
  const modal = bootstrap.Modal.getInstance(
    document.getElementById("withdrawPaymentModal"),
  );
  if (modal) {
    modal.hide();
  }
...
function editPayment(paymentFirestoreId) {
  const isAdmin = window.location.pathname.includes("management.html") || window.location.href.includes("management.html");
  if (!isAdmin && window.editAuthFlag === 0) {
    showAuthModal(() => editPayment(paymentFirestoreId), "Password for Payment Edit");
    return;
  }

  const payment = window.paymentsData.find(
    (p) => p.firestoreId === paymentFirestoreId,
  );
  if (!payment) {
  ...
  document.getElementById("editPaymentStudentFirestoreId").value =
    payment.studentFirestoreId || "";
  document.getElementById("editPaymentAmount").value = formatAmount(
    payment.amount,
  );
  ...
  if (payment.studentId && payment.studentName) {
    document.getElementById("editPaymentStudentDisplay").value =
      `${payment.studentId} - ${payment.studentName}`;
  } else {
    document.getElementById("editPaymentStudentDisplay").value =
...
function savePaymentEdit() {
  ...
  const originalAmount =
    parseFloat(document.getElementById("editPaymentOriginalAmount").value) || 0;
  const studentFirestoreId = document.getElementById(
    "editPaymentStudentFirestoreId",
  ).value;
  const newAmount = parseAmount(
...
function deletePayment(paymentFirestoreId) {
  const isAdmin = window.location.pathname.includes("management.html") || window.location.href.includes("management.html");
  if (!isAdmin && window.editAuthFlag === 0) {
    showAuthModal(() => deletePayment(paymentFirestoreId), "Password for Deletion");
    return;
  }
...
function viewPaymentDetails(paymentFirestoreId) {
  ...
  const amountPrefix = isWithdrawal ? "Withdrawal" : "Payment Received";
  const amountIcon = isWithdrawal ? "bi-dash-circle-fill" : "bi-check-circle-fill";
  ...
  const studentName = payment.studentName && payment.studentId
    ? `${payment.studentId} — ${payment.studentName}`
    : "General Transaction";

  const notesHtml = payment.notes
    ? `<div class="payment-detail-notes mt-3 p-3 rounded" style="background: rgba(255, 255, 255, 0.05); border-left: 4px solid var(--accent-primary, #0078d4);">
         <span class="d-block small text-secondary fw-bold text-uppercase mb-1">Notes / Description</span>
         <span style="font-size: 0.95rem; line-height: 1.5; color: var(--text-primary); white-space: pre-wrap;">${payment.notes}</span>
       </div>`
    : "";

  const contentHTML = `
    <div class="text-center py-4 border-bottom border-secondary mb-4">
        <div class="mb-3 d-inline-flex align-items-center justify-content-center rounded-circle" style="width: 72px; height: 72px; background: ${isWithdrawal ? 'rgba(209, 52, 56, 0.15)' : 'rgba(16, 124, 16, 0.15)'}; border: 2px solid ${isWithdrawal ? 'rgba(209, 52, 56, 0.3)' : 'rgba(16, 124, 16, 0.3)'};">
            <i class="bi ${amountIcon} ${isWithdrawal ? 'text-danger' : 'text-success'}" style="font-size: 2.2rem;"></i>
        </div>
        <div class="small text-secondary fw-bold text-uppercase tracking-wider mb-1">${amountPrefix}</div>
        <h2 class="fw-extrabold mb-0 ${amountClass}" style="font-size: 2rem; letter-spacing: -0.5px;">${amountSign}${amountFormatted}</h2>
    </div>

    <div class="payment-detail-grid d-flex flex-column gap-3">
        <div class="d-flex justify-content-between align-items-center">
            <span class="text-secondary small fw-bold text-uppercase">Student / Account</span>
            <span class="fw-semibold text-end text-truncate ms-3" style="max-width: 65%;" title="${studentName}">${studentName}</span>
        </div>
        <div class="d-flex justify-content-between align-items-center">
            <span class="text-secondary small fw-bold text-uppercase">Date & Time</span>
            <span class="fw-semibold">${dateStr}</span>
        </div>
        <div class="d-flex justify-content-between align-items-center">
            <span class="text-secondary small fw-bold text-uppercase">Payment Method</span>
            <span class="badge rounded-pill bg-primary px-3 py-1.5 fw-semibold" style="font-size: 0.78rem;">${payment.method || '—'}</span>
        </div>
        <div class="d-flex justify-content-between align-items-center">
            <span class="text-secondary small fw-bold text-uppercase">Processed By</span>
            <span class="badge rounded-pill bg-secondary px-3 py-1.5 fw-semibold" style="font-size: 0.78rem;">${payment.receivedBy || '—'}</span>
        </div>
        <div class="d-flex justify-content-between align-items-center">
            <span class="text-secondary small fw-bold text-uppercase">Transaction ID</span>
            <code class="text-secondary small text-end text-truncate ms-3" style="max-width: 50%; font-family: monospace;">${payment.firestoreId || '—'}</code>
        </div>
        ${notesHtml}
    </div>
  `;

  document.getElementById("viewPaymentDetailsContent").innerHTML = contentHTML;

  // Set action buttons handlers
  const editBtn = document.getElementById("viewPaymentEditBtn");
  const deleteBtn = document.getElementById("viewPaymentDeleteBtn");
  const printBtn = document.getElementById("viewPaymentPrintBtn");
    ...
    editBtn.onclick = () => {
      ...
      setTimeout(() => {
        if (typeof editPayment === "function") {
          editPayment(paymentFirestoreId);
        }
      }, 350);
    };
  }
    ...
    deleteBtn.onclick = () => {
      ...
      setTimeout(() => {
        if (typeof deletePayment === "function") {
          deletePayment(paymentFirestoreId);
        }
      }, 350);
    };
  }
  ...
  if (printBtn) {
    printBtn.onclick = () => {
      if (typeof printPaymentCheque === "function") {
        printPaymentCheque(paymentFirestoreId);
      }
    };
  }
...
async function printPaymentCheque(paymentFirestoreId) {
  const payment = window.paymentsData.find(
    (p) => p.firestoreId === paymentFirestoreId,
  );
  if (!payment) {
  ...
  // Fetch logo and convert to base64 so it works reliably in popup windows
  let logoDataUrl = '';
  try {
    ...
    logoDataUrl = ''; // logo simply won't show if fetch fails
  }

  // Build receipt number from firestoreId (last 8 chars)
  const receiptNo = paymentFirestoreId
    ? paymentFirestoreId.slice(-8).toUpperCase()
    : "N/A";
  ...
  const notesLine = payment.notes ? payment.notes : "";
  const methodLine = payment.method || "—";
  const receiverLine = payment.receivedBy || "—";
...
<body>
<div class="receipt">
    ${logoDataUrl ? `<img class="logo" src="${logoDataUrl}" alt="UNIAPP Logo">` : ''}
    ...
    <div class="row">
        ...
        <span class="value bold">${dateStr}</span>
    </div>
    <div class="row">
    ...
    <div class="row">
        ...
        <span class="value bold">${studentLine}</span>
    </div>
    ...
    <div class="row">
        ...
        <span class="value">${methodLine}</span>
    </div>
    <div class="row">
        <span class="label">Received by:</span>
        <span class="value">${receiverLine}</span>
    </div>
    ${notesLine ? `
    <div class="notes-row">
        <span style="font-size:9px; letter-spacing:1px; color:#444;">NOTE: </span>${notesLine}
    </div>` : ''}
    ...
    <div class="footer">
        ★  RAHMAT! THANK YOU!  ★<br>
        <span style="font-size:8px; color:#555;">unibridge.uz</span>
    </div>
</div>
</body>
</html>`;
...
window.printPaymentCheque = printPaymentCheque;
...
function renderTariffsList() {
  ...
  container.innerHTML = window.tariffsData
    .map(
      (t) => `
...
function saveTariff() {
  ...
  const price = parseInt(
    document.getElementById("tariffPrice").value.replace(/\D/g, ""),
    10,
  );
  ...
  const modal = bootstrap.Modal.getInstance(
    document.getElementById("tariffModal"),
  );
  if (modal) modal.hide();
}
...
function renderLevelsList() {
  ...
  container.innerHTML = window.levelsData
    .map(
      (l) => `
        <div class="settings-item">
            <div class="settings-item-info">
...
function openLevelModal(levelId = null) {
  ...
  if (levelId) {
    ...
    if (level) {
      title.innerHTML =
        '<i class="bi bi-mortarboard-fill me-2"></i>Edit Education Level';
      document.getElementById("levelEditId").value = levelId;
      document.getElementById("levelName").value = level.name;
    }
  } else {
    title.innerHTML =
      '<i class="bi bi-mortarboard-fill me-2"></i>Add Education Level';
  }
...
function renderGroupsList() {
  ...
  container.innerHTML = window.groupsData
    .map(
      (g) => `
        <div class="settings-item">
            <div class="settings-item-info">
...
function updateGroupDropdowns() {
  const groupSelects = document.querySelectorAll(
    "#filterGroup, #statusFilterGroup, #excelGroupFilter, #paymentGroupFilter, #documentsFilterGroup",
  );
...
let universitiesExpanded = false;

function toggleUniversitiesExpanded() {
...
function renderUniversitiesList() {
  ...
  const sortedUnis = [...window.universitiesData].sort((a, b) =>
    a.name.localeCompare(b.name)
  );
  const displayUnis = universitiesExpanded ? sortedUnis : sortedUnis.slice(0, 10);

  let html = '<div class="settings-list-container">';

  displayUnis.forEach((u) => {
    html += `
        <div class="settings-item" style="margin-bottom: 0.5rem; background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 0.75rem 1rem; display: flex; justify-content: space-between; align-items: center;">
            <div class="settings-item-info d-flex align-items-center">
                <i class="bi bi-building me-3 text-primary" style="font-size: 1.1rem;"></i>
                <span class="settings-item-name fw-semibold" style="font-size: 0.95rem;">${u.name}</span>
            </div>
            <div class="settings-item-actions d-flex gap-1">
                <button class="settings-item-btn edit-btn btn btn-sm btn-outline-secondary rounded-circle" onclick="editUniversity('${u.firestoreId}')" title="Edit">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="settings-item-btn delete-btn btn btn-sm btn-outline-danger rounded-circle" onclick="confirmDeleteUniversity('${u.firestoreId}')" title="Delete">
                    <i class="bi bi-trash"></i>
                </button>
            </div>
        </div>
  ...
  if (sortedUnis.length > 10) {
    const remaining = sortedUnis.length - 10;
    html += `
        <div class="text-center mt-3 mb-2">
            <button class="btn btn-outline-secondary rounded-pill px-4 py-2 fw-semibold shadow-sm" onclick="toggleUniversitiesExpanded()" style="font-size: 0.9rem; transition: all 0.2s;">
                ${
                  universitiesExpanded
                    ? '<i class="bi bi-chevron-up me-2"></i>Show less'
                    : `<i class="bi bi-chevron-down me-2"></i>Show more (${remaining} more)`
                }
            </button>
        </div>
    `;
...
function openUniversityModal(universityId = null) {
  ...
  if (universityId) {
    const uni = window.universitiesData.find(
      (u) => u.firestoreId === universityId,
    );
    if (uni) {
...
function saveUniversity() {
  ...
  if (!name) {
    showNotification("Please enter university name!", "error");
    return;
  }
  ...
  universitiesExpanded = true; // Auto-expand list so newly saved university is visible
...
function updateLevelDropdowns() {
  const levelSelects = document.querySelectorAll(
    "#levelSelect, #filterLevel, #statusFilterLevel, #excelLevelFilter, #documentsFilterLevel",
  );
  ...
  levelSelects.forEach((select) => {
    ...
    // Add "No Level" and deleted students option for filterLevel, statusFilterLevel and documentsFilterLevel
    if (select.id === "filterLevel" || select.id === "statusFilterLevel" || select.id === "documentsFilterLevel") {
      select.innerHTML +=
...
function updateUniversityDropdowns(selectedLevel) {
  const uniSelects = document.querySelectorAll(
    "#uni1, #uni2, #edit-university1, #edit-university2",
  );

  // Remove duplicates by name and sort alphabetically
  const uniqueUnis = Array.from(
    new Map((window.universitiesData || []).map((u) => [u.name, u])).values(),
...
window.toggleUniversitiesExpanded = toggleUniversitiesExpanded;
...
function renderVideosList() {
  ...
  container.innerHTML = window.videosData
    .map(
      (v) => `
        <div class="settings-item">
            <div class="settings-item-info">
...
function openVideoModal(videoId = null) {
  ...
  if (videoId) {
    ...
    if (video) {
      title.innerHTML =
        '<i class="bi bi-play-circle-fill me-2" style="color: #ef4444;"></i>Edit Video';
    ...
    title.innerHTML =
      '<i class="bi bi-play-circle-fill me-2" style="color: #ef4444;"></i>Add Video';
  }
...
function updateAdmissionUniversities() {
  ...
  const universities = getUniversitiesForLevel();
...
function generateRoundDatePickers() {
  const roundsCount = parseInt(
    document.getElementById("admissionRounds").value,
  );
...
function getLatestDate(rounds) {
  ...
  rounds.forEach((round) => {
    const dates = [
      round.onlineApplication,
      round.documentSubmission,
      round.interview,
      round.announcement,
    ];
    dates.forEach((date) => {
...
function renderAdmissions() {
  ...
  admissions.forEach((admission, index) => {
    ...
    if (admission.rounds && admission.rounds.length > 0) {
      roundsHtml = admission.rounds
        .map((round) => {
          const fromDate = round.onlineApplicationFrom
            ? formatDisplayDate(round.onlineApplicationFrom)
            : round.onlineApplication
              ? formatDisplayDate(round.onlineApplication)
              : "?";
          const toDate = round.onlineApplicationTo
            ? formatDisplayDate(round.onlineApplicationTo)
            : "?";
...
function filterAdmissions() {
  ...
  if (admissions.length === 0) {
    admissionsList.innerHTML =
      '<div class="col-12 text-center py-5"><i class="bi bi-inbox icon-empty-state"></i><p class="text-secondary mt-3">No admissions yet. Click "Add Admission" to get started.</p></div>';
    admissionsCounter.textContent = "No admissions";
    return;
  }
  ...
  if (filteredAdmissions.length === 0) {
    ...
    admissionsList.innerHTML =
      '<div class="col-12 text-center py-5"><i class="bi bi-search icon-empty-state" style="font-size: 3rem; opacity: 0.5;"></i><p class="text-secondary mt-3">' +
      message +
      "</p></div>";
  ...
  filteredAdmissions.forEach(function (admission) {
    ...
    if (admission.rounds && admission.rounds.length > 0) {
      admission.rounds.forEach(function (round) {
        var fromDate = round.onlineApplicationFrom
          ? formatDisplayDate(round.onlineApplicationFrom)
          : round.onlineApplication
            ? formatDisplayDate(round.onlineApplication)
            : "?";
        var toDate = round.onlineApplicationTo
          ? formatDisplayDate(round.onlineApplicationTo)
          : "?";

        if (round.onlineApplicationFrom || round.onlineApplicationTo) {
          roundsHtml +=
            '<div class="d-flex align-items-center justify-content-between py-2 px-3 mb-2 rounded-2" style="background: rgba(var(--bs-primary-rgb), 0.05); border-left: 3px solid var(--bs-primary);"><div class="d-flex align-items-center gap-2"><span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-2 py-1" style="font-size: 0.7rem;">R' +
            round.roundNumber +
            '</span><span class="text-body fw-medium" style="font-size: 0.85rem;">' +
            fromDate +
            '</span></div><div class="d-flex align-items-center gap-2"><i class="bi bi-arrow-right text-primary" style="font-size: 0.75rem;"></i><span class="text-body fw-medium" style="font-size: 0.85rem;">' +
            toDate +
            "</span></div></div>";
        } else if (round.onlineApplication) {
          roundsHtml +=
            '<div class="d-flex align-items-center gap-2 py-2 px-3 mb-2 rounded-2" style="background: rgba(var(--bs-primary-rgb), 0.05); border-left: 3px solid var(--bs-primary);"><span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-2 py-1" style="font-size: 0.7rem;">R' +
            round.roundNumber +
            '</span><span class="text-body fw-medium" style="font-size: 0.85rem;">' +
            fromDate +
            "</span></div>";
    ...
    html +=
      '<div class="col-md-6 col-lg-4 mb-3"><div class="card-apple h-100 cursor-pointer hover-lift" onclick="viewAdmissionDetails(' +
      index +
      ')" style="padding: 0; overflow: hidden;"><div class="p-3 pb-2" style="background: linear-gradient(135deg, rgba(var(--bs-primary-rgb), 0.08) 0%, rgba(var(--bs-info-rgb), 0.05) 100%); border-bottom: 1px solid rgba(var(--bs-border-color-rgb), 0.1);"><div class="d-flex align-items-start justify-content-between mb-2"><div class="d-flex align-items-center gap-2"><div class="bg-primary bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center" style="width: 36px; height: 36px; min-width: 36px;"><i class="bi bi-building text-primary" style="font-size: 1rem;"></i></div><div><h6 class="mb-0 fw-bold text-body" style="font-size: 0.95rem; line-height: 1.3;">' +
      admission.universityName +
      '</h6></div></div><span class="badge rounded-pill px-2 py-1" style="background: var(--bs-info); color: white; font-size: 0.7rem;">' +
      roundsCount +
      " " +
      (roundsCount === 1 ? "Round" : "Rounds") +
      '</span></div><div class="d-flex align-items-center gap-2 mt-2"><i class="bi bi-mortarboard text-secondary" style="font-size: 0.85rem;"></i><span class="text-secondary" style="font-size: 0.85rem;">' +
      (admission.educationLevel || "-") +
      "</span></div></div>" +
      (roundsHtml
        ? '<div class="p-3" style="max-height: 200px; overflow-y: auto;">' +
          roundsHtml +
          "</div>"
        : '<div class="p-3 text-center"><span class="text-secondary fst-italic" style="font-size: 0.85rem;">No application periods set</span></div>') +
      "</div></div>";
  });
...
function saveAdmission() {
  ...
  const information = document
    .getElementById("admissionInformation")
    .value.trim();
  const majors = document.getElementById("admissionMajors").value.trim();
  const scholarships = document
    .getElementById("admissionScholarships")
    .value.trim();
  ...
  if (editId) {
    ...
    if (typeof updateAdmissionInFirestore === "function" && firestoreId) {
      ...
      localStorage.setItem(
        "admissionsData",
        JSON.stringify(window.admissionsData),
      );
    }
...
function viewAdmissionDetails(index) {
  ...
  let detailsHtml =
    '<div class="d-flex align-items-center mb-4 ps-1"><div class="bg-primary bg-opacity-10 p-3 rounded-4 me-3 d-flex align-items-center justify-content-center shadow-sm" style="width: 56px; height: 56px; min-width: 56px;"><i class="bi bi-building text-primary fs-3"></i></div><div><h4 class="mb-1 fw-bold text-body tracking-wide">' +
    admission.universityName +
    '</h4><div class="d-flex align-items-center gap-2"><span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-10 fw-medium px-2 py-1">' +
    admission.educationLevel +
    "</span></div></div></div>";
  if (admission.rounds && admission.rounds.length > 0) {
    detailsHtml +=
    ...
    admission.rounds.forEach(function (round) {
      ...
      if (round.onlineApplicationFrom || round.onlineApplicationTo) {
        ...
        var toDate = round.onlineApplicationTo
          ...
          : "?";
        onlineAppDisplay =
          '<div class="d-flex flex-column" style="line-height: 1.2;"><span class="fw-medium text-body">' +
          fromDate +
          '</span><span class="text-secondary small" style="font-size: 0.75em; margin-top: 2px;">to ' +
          toDate +
          "</span></div>";
      } else if (round.onlineApplication) {
        onlineAppDisplay =
      ...
      var docDisplay = round.documentSubmission
        ? formatDisplayDate(round.documentSubmission)
        : '<span class="text-secondary opacity-50">-</span>';
      var interviewDisplay = round.interview
        ? formatDisplayDate(round.interview)
        : '<span class="text-secondary opacity-50">-</span>';
      var resultDisplay = round.announcement
        ? formatDisplayDate(round.announcement)
        : '<span class="text-secondary opacity-50">-</span>';
      detailsHtml +=
        '<tr><td class="ps-4 py-3"><span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-2 border border-primary border-opacity-10">R' +
        round.roundNumber +
        '</span></td><td class="py-3">' +
        onlineAppDisplay +
        '</td><td class="py-3 text-body opacity-75">' +
        docDisplay +
        '</td><td class="py-3 text-body opacity-75">' +
        interviewDisplay +
        '</td><td class="pe-4 py-3 text-body opacity-75">' +
        resultDisplay +
        "</td></tr>";
    });
    ...
    detailsHtml +=
      '<div class="text-center py-4 text-secondary fst-italic border border-dashed border-light-subtle rounded-3 mb-4">No rounds configured</div>';
  }
  ...
  var modal = new bootstrap.Modal(
    document.getElementById("viewAdmissionModal"),
  );
  modal.show();
}
...
function editAdmission() {
  ...
  setTimeout(() => {
    // Populate edit form
    document.getElementById("admissionEducationLevel").value =
      admission.educationLevel;
    updateAdmissionUniversities();

    setTimeout(() => {
      document.getElementById("admissionUniversity").value =
        admission.universityName;
        ...
        if (admission.rounds) {
          admission.rounds.forEach((round) => {
            ...
            if (document.getElementById(`round${i}OnlineAppFrom`)) {
              document.getElementById(`round${i}OnlineAppFrom`).value =
                round.onlineApplicationFrom || round.onlineApplication || "";
              document.getElementById(`round${i}OnlineAppTo`).value =
                round.onlineApplicationTo || "";
              document.getElementById(`round${i}DocSubmission`).value =
                round.documentSubmission || "";
              document.getElementById(`round${i}Interview`).value =
                round.interview || "";
              document.getElementById(`round${i}Announcement`).value =
                round.announcement || "";
            }
          });
        }
    ...
    document.getElementById("admissionInformation").value =
      admission.information || "";
    document.getElementById("admissionMajors").value = admission.majors || "";
    document.getElementById("admissionScholarships").value =
...
function deleteAdmission() {
  ...
  if (
    !confirm(
      `Are you sure you want to delete this admission record for "${admission.universityName}"?`,
    )
  ) {
    return;
  }
  ...
  if (
    typeof deleteAdmissionFromFirestore === "function" &&
...
function saveNotification() {
  ...
  const durationFrom = document.getElementById(
    "notificationDurationFrom",
  ).value;
  ...
  if (editId) {
    ...
    if (typeof saveNotificationToFirestore === "function") {
      ...
      localStorage.setItem(
        "notificationsData",
        JSON.stringify(window.notificationsData),
      );
...
function editNotification() {
  ...
  document.getElementById("notificationDurationFrom").value =
    notification.durationFrom || "";
  document.getElementById("notificationDurationTo").value =
...
function deleteNotification() {
  ...
  if (
    typeof deleteNotificationFromFirestore === "function" &&
...
window.viewDocumentsModal = viewDocumentsModal;
window.togglePickNeeded = togglePickNeeded;
window.toggleMcEnabled = toggleMcEnabled;
window.updateCopyCount = updateCopyCount;
window.resetCopyCount = resetCopyCount;
window.populateStudentDropdown = populateStudentDropdown;
window.selectStudentForPayment = selectStudentForPayment;
window.addQuickNote = addQuickNote;
window.submitPayment = submitPayment;
...
function togglePremiumAccordion(id) {
  ...
  const allItems = item.parentElement.querySelectorAll(
...
function showVisaSubTab(tabName) {
  ...
  document
    .querySelectorAll("#visastatus-tab .btn-group button")
    .forEach((btn) => {
      btn.classList.remove("active");
    });
  const activeBtn = document.getElementById(
    `visa${tabName.charAt(0).toUpperCase() + tabName.slice(1)}TabBtn`,
  );
  if (activeBtn) {
...
async function saveVisaStatusToFirestore(studentId, statusData) {
  if (typeof saveVisaStatusToFirestoreConfig === "function") {
    ...
    localStorage.setItem(
      "visaStatusData",
      JSON.stringify(window.visaStatusData),
    );
    renderVisaStatusLists();
    updateVisaTabCounts();
  }
...
function renderVisaAllStudentsList() {
  ...
  const students = (window.studentsData || []).filter((s) => {
    ...
    if (visaStatusEntry) {
      ...
      console.log(
        `   ❌ EXCLUDED - Already has visa status: ${statusData.status}`,
      );
      return false;
    }
    ...
    console.log(
      `   Checking eduLevel: "${eduLevel}" (length: ${eduLevel.length})`,
    );
  ...
  // Debug log
  const totalStudents = (window.studentsData || []).filter(
    (s) => !s.isDeleted,
  ).length;
  console.log(
    `📊 Visa Students Filter: ${students.length}/${totalStudents} students shown (${Object.keys(window.visaStatusData || {}).length} have visa status)`,
  );

  if (students.length === 0) {
    container.innerHTML =
      '<div class="text-center py-4 text-secondary"><i class="bi bi-check-circle me-2"></i>All students have been checked or are Masters level</div>';
    return;
  }
  ...
  students.forEach((student) => {
    ...
    const hasRequiredData =
      student.passport && student.fullName && student.birthday;
    ...
    const hasUnknownStatus =
      visaStatusEntry && visaStatusEntry[1].status === "UNKNOWN";
...
function filterVisaAllStudents() {
  ...
  const cards = document.querySelectorAll(
    "#visaAllStudentsList .visa-student-check-card",
  );
...
async function checkSingleStudentVisa(studentId) {
  ...
  if (!student.passport || !student.fullName || !student.birthday) {
    showNotification(
      "Missing required data (passport, name, or birthday)",
      "error",
    );
    return;
  }
  ...
  const existingStatus = window.visaStatusData
    ? window.visaStatusData[studentId]
    : null;
  ...
  let card = document.querySelector(
    `.visa-student-check-card[data-student-id="${studentId}"] .check-btn`,
  );
  if (!card) {
    card = document.querySelector(
      `.visa-tracker-card .recheck[onclick*="${studentId}"]`,
    );
  }
  ...
  try {
    ...
    const statusChanged =
      previousStatus && previousStatus !== visaResult.status;

    if (statusChanged) {
      // Status changed - DO NOT SAVE YET, just show Move button
      console.log(
        `🔄 Status changed: ${previousStatus} → ${visaResult.status}`,
      );
      ...
      if (visaResult.status === "APPROVED") {
        ...
        visaResult.status === "CANCELLED" ||
        visaResult.status === "REJECTED"
      ) {
        targetTab = "cancelled";
      ...
      const cardElement = document
        .querySelector(
          `.visa-tracker-card [onclick*="recheckStudentVisa('${studentId}')"]`,
        )
        ?.closest(".visa-tracker-card");
      if (cardElement) {
      ...
      showNotification(
        `Status changed: ${previousStatus} → ${visaResult.status}. Click the arrow button to move the card.`,
        "info",
      );
    } else {
      ...
      const needsManualMove =
        isFirstTimeCheck &&
        (visaResult.status === "APPROVED" ||
          visaResult.status === "CANCELLED" ||
          visaResult.status === "REJECTED");
      ...
      if (needsManualMove) {
        ...
        console.log(
          `🆕 First check with ${visaResult.status} status - saving to App/Received with move option`,
        );
        ...
        showNotification(
          `Visa status: ${visaResult.status}. Review and move to appropriate tab.`,
          "info",
        );
        ...
        setTimeout(() => {
          ...
          if (cardElement) {
            ...
            if (actionsDiv) {
              ...
              moveBtn.onclick = () =>
                confirmAndMoveVisaCard(studentId, targetTab, finalStatusData);
        ...
        showNotification(
          `Visa status: ${visaResult.status}`,
          visaResult.status === "APPROVED" ? "success" : "info",
        );
        ...
        showNotification(
          "Visa status: UNKNOWN - Student will remain in the list",
          "warning",
        );
      }
    }
    ...
    showNotification(
      "API error. Click the button again or check manually on visa.go.kr",
      "error",
    );
  ...
  if (card) {
    card.innerHTML =
      originalButtonContent || '<i class="bi bi-arrow-clockwise"></i>';
    card.disabled = false;
...
function renderVisaStatusLists() {
  ...
  // Filter by status
  const receivedStudents = statusEntries.filter(
    (s) =>
      s.status === "APP/RECEIVED" ||
      s.status === "UNDER REVIEW" ||
      s.status === "PENDING",
  );
  const cancelledStudents = statusEntries.filter(
  ...
  if (receivedList) {
    if (receivedStudents.length === 0) {
      receivedList.innerHTML =
        '<div class="text-center py-4 text-secondary"><i class="bi bi-inbox"></i> No students with App/Received status yet</div>';
    } else {
      receivedList.innerHTML = receivedStudents
        .map((s) => renderVisaTrackerCard(s, "status-received"))
        .join("");
    }
  ...
  if (cancelledList) {
    if (cancelledStudents.length === 0) {
      ...
      cancelledList.innerHTML = cancelledStudents
        .map(
          (s) => `
  ...
  if (approvedList) {
    if (approvedStudents.length === 0) {
      ...
      approvedList.innerHTML = approvedStudents
        .map(
          (s) => `
                <div class="col-md-6 col-lg-4 visa-status-card-wrapper" data-search-text="${(s.studentName || "").toLowerCase()} ${(s.studentId || "").toLowerCase()}">
...
function renderVisaTrackerCard(statusData, statusClass) {
  ...
  if (statusData.originalStatus && statusData.status === "APP/RECEIVED") {
    ...
    if (statusData.originalStatus === "APPROVED") {
      ...
      statusData.originalStatus === "CANCELLED" ||
      statusData.originalStatus === "REJECTED"
    ) {
      targetTab = "cancelled";
      targetTabName = "Cancelled";
      moveButtonColor = "danger";
    ...
    if (targetTab) {
      ...
      const encodedStatusData = encodeURIComponent(
        JSON.stringify(finalStatusData),
      );
...
function removeVisaStatus(studentId) {
  if (
    !confirm(
      "Remove this visa status from the list? (Student data will NOT be deleted)",
    )
  ) {
    return;
  }
...
async function confirmAndMoveVisaCard(studentId, targetTab, statusData) {
  try {
    ...
    showNotification(
      `Student moved to ${targetTab.charAt(0).toUpperCase() + targetTab.slice(1)} tab!`,
      "success",
    );
...
function updateVisaTabCounts() {
  ...
  const receivedCount = statusEntries.filter(
    (s) =>
      s.status === "APP/RECEIVED" ||
      s.status === "UNDER REVIEW" ||
      s.status === "PENDING",
  ).length;
  const cancelledCount = statusEntries.filter(
    (s) => s.status === "CANCELLED" || s.status === "REJECTED",
  ).length;
  const approvedCount = statusEntries.filter(
    (s) => s.status === "APPROVED",
  ).length;
...
function filterVisaReceivedStudents() {
  ...
  const searchTerm = searchInput ? searchInput.value.toLowerCase().trim() : "";
  const cards = document.querySelectorAll(
    "#visaReceivedList .visa-tracker-card",
  );
  ...
  cards.forEach((card) => {
    const name = card.querySelector(".student-name")
      ...
      : "";
    const passport = card.querySelector(".student-passport")
      ? card.querySelector(".student-passport").textContent.toLowerCase()
      : "";
    card.style.display =
      name.includes(searchTerm) || passport.includes(searchTerm)
        ? "block"
        : "none";
  });
}
...
function filterVisaCancelledStudents() {
  ...
  const wrappers = document.querySelectorAll(
    "#visaCancelledList .visa-status-card-wrapper",
  );
...
function filterVisaApprovedStudents() {
  ...
  const searchTerm = searchInput ? searchInput.value.toLowerCase().trim() : "";
  const wrappers = document.querySelectorAll(
    "#visaApprovedList .visa-status-card-wrapper",
  );
...
async function fetchVisaStatus(student) {
  ...
  throw new Error(
    "All CORS proxies failed. Please try again later or check your internet connection.",
  );
}
...
function translateVisaStatus(status) {
  ...
  if (
    ...
    statusUpper.includes("TASDIQLANGAN")
  ) {
    return "APPROVED";
  }
  if (
