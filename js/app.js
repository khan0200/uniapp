...
function getUniversitiesForLevel(level) {
  ...
  const universities = window.universitiesData
    .filter(
      (u) =>
        u.levelName && u.levelName.trim().toUpperCase() === normalizedLevel,
    )
    .map((u) => u.name);

  if (universities.length === 0) {
    console.log(
      `ℹ️ No universities found for level "${level}". Please add universities for this level in Settings → Universities.`,
    );
  }
  return universities;
}
...
function renderUniversitiesList() {
  ...
  console.log("📚 Unique levelNames:", [
    ...new Set(window.universitiesData.map((u) => u.levelName)),
  ]);
...
function updateTariffDropdowns() {
  const tariffSelects = document.querySelectorAll(
    "#tariff, #filterTariff, #statusFilterTariff, #paymentTariffFilter",
  );
...
function updateLevelDropdowns() {
  const levelSelects = document.querySelectorAll(
    "#levelSelect, #filterLevel, #statusFilterLevel, #excelLevelFilter",
  );
...
function updateUniversityDropdowns(selectedLevel) {
  ...
  const normalizedLevel = selectedLevel
    ? selectedLevel.trim().toUpperCase()
    : "";
  const filteredUnis = normalizedLevel
    ? window.universitiesData.filter(
        (u) =>
          u.levelName && u.levelName.trim().toUpperCase() === normalizedLevel,
      )
    : [];
  ...
  const uniqueUnis = Array.from(
    new Map(filteredUnis.map((u) => [u.name, u])).values(),
  );
