...
function loadUniversitiesFromFirestore() {
    if (!firebaseInitialized) return;
    console.log('🔄 Loading universities from Firestore...');
    
    db.collection('listofuniversities').get().then(snapshot => {
        if (snapshot.empty) {
            console.log('📦 Firestore listofuniversities collection is empty. Seed uploading defaultUniversities...');
            defaultUniversities.forEach(name => {
                db.collection('listofuniversities').add({
                    name,
                    levelId: 'GENERAL',
                    levelName: 'General',
                    createdAt: firebase.firestore.FieldValue.serverTimestamp()
            ...
            window.universitiesData = defaultUniversities.map((name, i) => ({
                firestoreId: 'temp-' + i,
                name,
                levelId: 'GENERAL',
                levelName: 'General'
        ...
        snapshot.forEach(doc => {
            ...
            if (data.name) {
                unis.push({
                    firestoreId: doc.id,
                    name: data.name,
                    levelId: data.levelId || 'GENERAL',
                    levelName: data.levelName || 'General',
                    ...data
                });
            }
        });
        ...
        if (typeof renderUniversitiesList === 'function') renderUniversitiesList();
        if (typeof updateUniversityDropdowns === 'function') updateUniversityDropdowns();
    }).catch(err => console.error('Error loading universities from Firestore:', err));
}
