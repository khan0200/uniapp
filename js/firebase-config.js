...
// ==========================================
// VISA STATUS FIRESTORE FUNCTIONS
// ==========================================

// Global visa status data object
window.visaStatusData = {};

// Load visa status from Firestore
function loadVisaStatusFromFirestoreConfig() {
    if (!firebaseInitialized) {
        console.log('Firebase not available, loading visa status from localStorage');
        const stored = localStorage.getItem('visaStatusData');
        if (stored) {
            window.visaStatusData = JSON.parse(stored);
        }
        // Trigger render if functions exist
        if (typeof renderVisaAllStudentsList === 'function') renderVisaAllStudentsList();
        if (typeof renderVisaStatusLists === 'function') renderVisaStatusLists();
        if (typeof updateVisaTabCounts === 'function') updateVisaTabCounts();
        return;
    }

    // Real-time listener for visaStatus collection
    db.collection('visaStatus')
        .onSnapshot((snapshot) => {
            window.visaStatusData = {};
            ...
            snapshot.forEach((doc) => {
                ...
                if (data.checkedAt && data.checkedAt.toDate) {
                    data.checkedAt = data.checkedAt.toDate().toISOString();
                }

                window.visaStatusData[doc.id] = {
                    ...data,
                    firestoreId: doc.id
                };
            });

            console.log(`✅ Loaded ${Object.keys(window.visaStatusData).length} visa statuses from Firestore`);
            ...
            console.error('❌ Error loading visa status from Firestore:', error);
            // Fallback to localStorage
            const stored = localStorage.getItem('visaStatusData');
            if (stored) {
                window.visaStatusData = JSON.parse(stored);
...
// Save visa status to Firestore
async function saveVisaStatusToFirestoreConfig(studentId, statusData) {
    if (!firebaseInitialized) {
        console.log('Firebase not available, saving visa status to localStorage');
    ...
    try {
        // Add timestamp
        statusData.checkedAt = firebase.firestore.FieldValue.serverTimestamp();

        await db.collection('visaStatus').doc(studentId).set(statusData, {
            merge: true
        });

        console.log('✅ Saved visa status to Firestore for:', studentId);

        // Update local data immediately (timestamp will be updated by snapshot)
        ...
        console.error('❌ Error saving visa status to Firestore:', error);
        // Fallback to localStorage
        window.visaStatusData[studentId] = statusData;
        localStorage.setItem('visaStatusData', JSON.stringify(window.visaStatusData));
    }
...
// Delete visa status from Firestore
async function deleteVisaStatusFromFirestoreConfig(studentId) {
    if (!firebaseInitialized) {
    ...
    try {
        await db.collection('visaStatus').doc(studentId).delete();

        console.log('✅ Deleted visa status from Firestore for:', studentId);
        ...
        console.error('❌ Error deleting visa status from Firestore:', error);
        // Fallback to localStorage
        delete window.visaStatusData[studentId];
        localStorage.setItem('visaStatusData', JSON.stringify(window.visaStatusData));
    }
...
// Export visa status functions
window.loadVisaStatusFromFirestoreConfig = loadVisaStatusFromFirestoreConfig;
window.saveVisaStatusToFirestoreConfig = saveVisaStatusToFirestoreConfig;
window.deleteVisaStatusFromFirestoreConfig = deleteVisaStatusFromFirestoreConfig;